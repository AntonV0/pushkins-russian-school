...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-20 border-b border-border-soft/80 bg-background/94 shadow-[0_1px_0_rgba(255,255,255,0.75)_inset] backdrop-blur-xl">
      <div className="mx-auto hidden max-w-7xl items-center justify-between gap-6 border-b border-border-soft/70 px-6 py-2 text-xs font-medium text-slate-600 xl:flex lg:px-8">
        <span>Weekend Russian classes for children aged 3-18</span>
        <span>Enquire about current places or future local classes</span>
      </div>
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-3 lg:px-8 lg:py-3.5">
        ...
        <nav
          ...
          className="hidden items-center gap-x-1 gap-y-3 text-sm font-semibold text-slate-700 xl:flex"
        >
          {primaryNavigation.map((item) => (
            <Link
              ...
              className={`relative rounded-md px-3 py-2 transition focus:outline-none focus:ring-2 focus:ring-brand-red/30 ${
                isActive(item.href)
                  ? "bg-white/80 text-brand-blue-strong shadow-[0_1px_0_rgba(255,255,255,0.75)_inset] after:absolute after:inset-x-3 after:-bottom-1 after:h-0.5 after:rounded-full after:bg-brand-red"
        ...
        <details ref={mobileMenuRef} className="group relative shrink-0 xl:hidden">
          <summary className="flex min-h-11 cursor-pointer list-none items-center justify-center gap-2 rounded-md border border-brand-blue/20 bg-white/80 px-3 py-2 text-sm font-semibold text-brand-blue-strong shadow-[0_1px_0_rgba(255,255,255,0.75)_inset] transition hover:border-brand-red hover:text-brand-red focus:outline-none focus:ring-2 focus:ring-brand-red/30">
          ...
          <nav
            ...
            className="premium-panel absolute right-0 top-12 z-30 max-h-[calc(100dvh-6rem)] w-[min(22rem,calc(100vw-2.5rem))] overflow-y-auto rounded-lg border border-border-soft bg-surface p-3 shadow-xl"
          >
              ...
              {primaryNavigation.map((item) => (
                <Link
                  ...
                  className={`rounded-md px-3 py-3 text-sm font-semibold transition focus:outline-none focus:ring-2 focus:ring-brand-red/30 ${
                    isActive(item.href)
                      ? "bg-surface-blue text-brand-blue-strong"
